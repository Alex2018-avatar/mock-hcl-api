...
export const generateOrderItem = async (orderItem, store: string) => {
  ...
  const fsPath = FsUtil.getFilePath(store, "cart/cart-item-base.json");
  const data = await FsUtil.readAndParseJSON(fsPath);
  console.log("data: ", data);
  const item = data ?? orderItemBase;
  return {
    ...item,
    partNumber,
    quantity,
...
export function updateCartWithNewItem(cart, newOrderItem) {
  ...
  const existingItem = orderItems.filter(
    (item) => item.partNumber === newOrderItem.partNumber
  );
  const restItems = orderItems.filter(
    (item) => item.partNumber !== newOrderItem.partNumber
  );
