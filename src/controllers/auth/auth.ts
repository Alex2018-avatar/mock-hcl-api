...
class AuthController {
  static async loginIdentity(req: Request, res: Response) {
  ...
  static async b2bLoginIdentity(req, res) {
    ...
    console.log("req.session.userAuth: ", req.session);
    req.session.userAuth = { userId, logonId };
    ...
    res.status(201).json({
      ...
      userId: userId,
      WCTrustedToken: `${userId}%2CGl%2BWZiFvtHRe%2F7YfZSsDG1D2%2FrrcSCEX08mjCcSRQ28%3D`,
      roleNavigationOnly: user.userId === "17003" ? "true" : "false",
    });
  }

  static async guestidentity(req: Request, res: Response) {
    const setCookie = (name, value, options = {}) =>
      res.cookie(name, value, { ...cookieOptions, ...options });
    setCookie("JSESSIONID", "0000KG3fhotHeqN7suRD0eve7pL%3A-1");
    setCookie(
      "WC_PERSISTENT",
      "ovTCjQhaLUDcbiznHAwzYciastaO8%2By1FeEeonkpq%2Fw%3D%3B2024-11-27%2B15%3A18%3A51.677_1732720731667-26_41",
      {
        httpOnly: true,
        maxAge: 3600000,
      }
    ...
    setCookie(
      "WC_AUTHENTICATION_38",
      "38%2CMu0yZ5KTjX0cRGD82HTiQZ9fy81H1ooosQVJLix0oMA%3D",
      {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        maxAge: 3600000,
    ...
    setCookie(
      "WC_USERACTIVITY_38",
      "38%2C41%2Cnull%2Cnull%2C1732720731678%2Cnull%2Cnull%2Cnull%2Cnull%2Cnull%2C1383256606%2Cver_1732720731633%2C9M8zbDT29v7zlMrmXfmc7sxu8zSMaXFV72Afbb7cud6QDJUIST6sKtR8MCAxFrS56nv2aOsbMhCsL2WJaR9PTB80WJaP01BcDkvKLCpBr114D43sp2khD71wHPBJI0ZeMb%2FINXwBje8YqgeK1e190Om9fUYBjPtYe%2BNacpWTdXy4D2P0YKmksUGnAtI%2B1y%2FRK%2FbK3IeAfkH3q%2BjI3kUJg0qFQvvbE0p4U98ylpqZ3bAxIr4pWMGEmwsoM1jS8mvZKxVQ2KZCqAwxu6dyh4h2cw%3D%3D",
      {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        maxAge: 3600000,
    ...
    res.status(201).json({
      ...
      WCToken:
        "37%2CL9efemNPxGeXUttnZcUM1xJo56S3SmfZloASjeUqPQlmmQJkzitlxnKrQ6uX7QEVb83y5W6rkNLvf3lvVIfWIRRthPJtFutSy%2BP9%2BxJulUe6%2BNQXslPCU7OmIWG37Y%2FWRz5qhmACHe195372nv7WRFRDCHsMwuJRKrKGk2t6yRj0GEuA%2FHSOuQL7mONd%2F2so0FYrtDSZO2kYeDmW%2B7nyw%2FnzUtnNCrMmOINAFWLMkGjCOUhKYdFLlV5kcNpZYx0e",
      userId: "37",
  ...
  static async logOut(req: Request, res: Response) {
    ...
    if (req.session?.userAuth) {
      ...
      res.clearCookie(`WC_AUTHENTICATION_${userId}`);
      res.clearCookie(`WC_USERACTIVITY_${userId}`);
    }
  ...
  static async syncRegister(req: Request, res: Response) {
    const { body } = req;
    const { logonId } = body;
  ...
  static async getNavigationRole(_req: Request, res: Response) {
    res.status(200).json({ roleNavigationOnly: true });
  }
}
