...
import { getAddressId } from "../../utils/address-utils";
import { ADDRESS_ID_NOT_AVAILABLE } from "../../config/promises";
...
export class PromisesController {
  static async getPromises(req, res) {
    ...
    try {
      // SUCCESS
      if (way === "SUCCESS") {
        const filePath = FileUtil.getFilePath(_folder, `promises/${way}.json`);
      ...
      // ERROR 500,408
      if (genericErrors.includes(way)) {
        const filePath = FileUtil.getFilePath(
          _folder,
          `promises/ERR_GENERIC_03.json`
        );
        const response = await FileUtil.readAndParseJSON(filePath);
      ...
      // DEFAULT SUCCESS
      const filePath = FileUtil.getFilePath(_folder, `promises/SUCCESS.json`);
      const response = await FileUtil.readAndParseJSON(filePath);
      return res.status(200).json(response);
    } catch (error) {
