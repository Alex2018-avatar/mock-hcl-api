...
} from "../../../helpers/order-item-generator.js";
...
cartRouter.get("/:storeId/cart/@selfXX", async (req, res, next) => {
  const demo = true;
  // if (demo) {
  //   res.status(401).json({
  //     "errors": [
...
cartRouter.post("/:storeId/carts", async (req, res) => {
  ...
  try {
    ...
    if (req.session.carts[storeId]) {
      ...
      const filePath = path.resolve(
        __dirname,
        `../../../data/${storeId}-store/cart/cart-base.json`
      );
      const data = await fsPromises.readFile(filePath, "utf8");
      const cart = JSON.parse(data);
      const newCart = {
        ...cart,
        orderItem: addItemToArray(cart.orderItem, generatedOrderItem),
      };
...
// /wcs/resources/store/10251/cart
cartRouter.put("/:storeId/cart/@self/update_order_itemz", async (req, res) => {
  const { storeId } = req.params;
...
cartRouter.get(
  "/:storeId/cart/@self/usable_shipping_info",
  addFolder,
  ShippingController.getUsableShippingInfo
);
...
cartRouter.get("/:storeId/cart/@self/usable_payment_info", async (req, res) => {
  ...
  const filePath = path.resolve(
    __dirname,
    `../../../data/${storeId}-store/cart/usable_payment_info.json`
  );
  const data = await fsPromises.readFile(filePath, "utf8");
...
// /wcs/resources/store/9701/cart/@self/payment_instruction?langId=-5
cartRouter.delete(
  "/:storeId/cart/@self/payment_instruction",
  async (req, res) => {
    res.status(200).json({});
  }
);
...
cartRouter.put(
  "/:storeId/cart/@self/delete_order_item",
  addFolder,
  CartController.deleteOrderItem
);

cartRouter.post(
  "/:storeId/cart/calculate",
  addFolder,
  CartController.cartCalculate
);
