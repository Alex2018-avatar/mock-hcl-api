...
import { getProductByIdOrPartNumber } from "../../../utils/product-utils";
import { FileUtil } from "../../../services/FileService";
import { addFolder } from "@middleware/auth";
import { getProducts } from "@controllers/search/products/product";
...
productsRouter.get("/products", addFolder, getProducts);
productsRouter.get("/productsX", async (req, res) => {
  ...
  try {
    if (categoryId) {
      const filePath = path.resolve(
        __dirname,
        `../../../data/${storeId}-store/search/${categoryId}-by-category.json`
      );
      const data = await fsPromises.readFile(filePath, "utf8");
      const response = JSON.parse(data);
      res.status(200).json(response);
    } else if (isPartNumberArray) {
      const products = await partNumber.reduce(
        async (accPromise, partNumber) => {
          const acc = await accPromise;
          const data = getProductByIdOrPartNumber(partNumber);
          const filePath = path.resolve(
            __dirname,
            `../../../data/${storeId}-store/product/${data.jsonName}.json`
          );
      ...
      const filePath = path.resolve(
        __dirname,
        `../../../data/${storeId}-store/search/3074457345616680171-by-search.json`
      );
      const data = await fsPromises.readFile(filePath, "utf8");
      const response = JSON.parse(data);
      res.status(200).json(response);
